...
```{r}
### Data Preparation
eccen4_myfiles = list.files(path = "../data/pilotA-eccen4" , pattern= "*.csv", full.names=TRUE)
eccen4_df = ldply(eccen4_myfiles, read_csv)
```
```{r}

eccen6_myfiles = list.files(path = "../data/pilotA-eccen6" , pattern= "*.csv", full.names=TRUE)
eccen6_df = ldply(eccen6_myfiles, read_csv)
```
...
```
```{r}
filtered_eccen4_df <- subset(filtered_eccen4_df, select= c(participantID, eccentricity_deg, critical_spacing_deg, crowding_session, correctRate))
```
```{r}
filtered_eccen6_df <- 
  eccen6_df  %>%
...
```
```{r}
filtered_eccen6_df <- subset(filtered_eccen6_df, select= c(participantID, eccentricity_deg, critical_spacing_deg, crowding_session, correctRate))
```
```{r}
visual_crowding <- rbind(filtered_eccen6_df, filtered_eccen4_df)
```
...
distance_eccen4_df <- subset(distance_eccen4_df, select= c(participantID,viewDistance_1, viewDistance_2, viewDistance_3 ))

distance_eccen6_df <- 
  eccen6_df  %>%
  filter(grepl("end_trial", trialType))
 
distance_eccen6_df <- subset(distance_eccen6_df, select= c(participantID,viewDistance_1, viewDistance_2, viewDistance_3 ))

viewing_distance_df<- rbind(distance_eccen4_df,distance_eccen6_df)
viewing_distance_df
```
